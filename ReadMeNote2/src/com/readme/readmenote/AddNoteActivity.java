...
public class AddNoteActivity extends Activity {
	...
	private void responseActivity(){
		...
		if (label == 2) {
			...
			addnote_record_btn.performClick();
		} else if (label == 5) {
			addnote_voice2text_btn.performClick();
		} else if (label == 6) {
			addnote_painting_btn.performClick();
		} else if (label == 7) {
			addnote_addRescource_btn.performClick();

		}
	}

	/**
	 * 初始化用户标题，详细内容，保存按钮，添加图片，录音，语音输入，涂鸦，添加附件，分享按钮。
	 * */
	...
	public void setButtonClickListener() {
		...
		note_content_et.setOnClickListener(new EditTextClickListner(AddNoteActivity.this, note_content_et));
	}
...
//	    		imageView.setImageBitmap(bitmap);
	  ...
	  private static Note createNote(String title,String content) throws Exception {
...
//=============================================================================
	...
	private void addResourceTag(File file,int scale){
		...
		UpLoadFileTask fileTask = new UpLoadFileTask(client,uiHandler,AddNoteActivity.this);
	...
	//===============================================================================
	private class UiAsyntask extends AsyncTask<String, Void, Note>{
...
//				DealString.cutString(params[1]);
				note = createNote(params[0], params[1]);
