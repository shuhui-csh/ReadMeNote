 ...
 */
public class Cloud_sharing_activity extends Activity implements OnClickListener {
	//定义图片存放的地址
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.example.readmenote.R.layout.cloud_share);
	...
	private void initView(){
		authLoginBtn = (Button)findViewById(R.id.btn_share_login);
		shareGuiBtn = (Button)findViewById(R.id.share_yes_share_xml);
		shareBtn = (Button)findViewById(R.id.btn_no_share_xml);
		getInfoBtn = (Button)findViewById(R.id.btnShareinfo);
	}
	...
	private void initImagePath() {
		try {//判断SD卡中是否存在此文件夹
			...
			if (!file.exists()) {
				file.createNewFile();
				Bitmap pic = BitmapFactory.decodeResource(getResources(), com.example.readmenote.R.drawable.pic);
	...
	public void onClick(View v) {
		...
		case R.id.btn_share_login://
			startActivity(new Intent(Cloud_sharing_activity.this,AuthActivity.class));
			break;
		case R.id.share_yes_share_xml:
			showGrid(false);
			break;
		case R.id.btn_no_share_xml:
			showGrid(true);
			break;
		case R.id.btnShareinfo:
			// 获取自己的资料
	...
	private void showGrid(boolean silent) {
		...
		i.putExtra("text", "http://note.youdao.com/share/?id=ce5436abe0eb6a9be1b4cfb31212d8f6&type=note");
		// imagePath是本地的图片路径，所有平台都支持这个字段，不提供，则表示不分享图片
		i.putExtra("imagePath", Cloud_sharing_activity.TEST_IMAGE);
		// url仅在微信（包括好友和朋友圈）中使用，否则可以不提供
		i.putExtra("url", "http://sharesdk.cn");
		// thumbPath是缩略图的本地路径，仅在微信（包括好友和朋友圈）中使用，否则可以不提供
		i.putExtra("thumbPath", Cloud_sharing_activity.TEST_IMAGE);
		// appPath是待分享应用程序的本地路劲，仅在微信（包括好友和朋友圈）中使用，否则可以不提供
		i.putExtra("appPath", Cloud_sharing_activity.TEST_IMAGE);
